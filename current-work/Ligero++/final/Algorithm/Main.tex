...
\begin{frame}{Inner product argument}
	...
	\begin{minipage}{0.42\linewidth}
	 	\includegraphics[scale=0.45]{ipa1.png}
	\end{minipage}
\end{frame}

\begin{frame}{Inner product argument}
	\begin{minipage}{0.42\linewidth}
			\includegraphics[scale=0.45]{ipa2.png}
	\end{minipage}
\end{frame}

\begin{frame}{Inner product argument}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.45]{ipa3.png}
	\end{minipage}
\end{frame}

\begin{frame}{Inner product argument}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.45]{ipa4.png}
	\end{minipage}
\end{frame}

\begin{frame}{Inner product argument}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.45]{ipa5.png}
	\end{minipage}
\end{frame}



\begin{frame}{Reed-Solomon Code Summary}
	We have a message $m=(m_1,\ldots,m_k)$ $\rightarrow$ We find polynomial $p$ such that 
	$\left(p\left(\zeta_1\right), \ldots, p\left(\zeta_k\right)\right)=(m_1,\ldots,m_k)$ $\rightarrow$  We define $\left(p\left(\eta_1\right), \ldots, p\left(\eta_n\right)\right)=(u_1,\ldots,u_n)=u$ as the encoded message of $m$  \\
\end{frame}


\begin{frame}{Reed-Solomon Code}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.65]{rs1.png}
	\end{minipage}
\end{frame}

\begin{frame}{Reed-Solomon Code}
	Let $L=R S_{\mathbb{F}, n, k, \eta}$ be an RS code and $\zeta=\left(\zeta_1, \ldots, \zeta_k\right)$ be a sequence of distinct elements in $\mathbb{F}$. 
\end{frame}

\begin{frame}{Reed-Solomon Code}
	For a codeword $u \in L$, we define the message $\operatorname{Dec}_\zeta(u)$ to be $\left(p_u\left(\zeta_1\right), \ldots, p_u\left(\zeta_k\right)\right)$, where $p_u$ is the polynomial (of degree $<k$ ) corresponding to $u$ such that:
	$$\left(p_u\left(\eta_1\right), \ldots, p_u\left(\eta_n\right)\right)=(u_1,\ldots,u_n)$$
\end{frame}


\begin{frame}{Reed-Solomon Code Extended Version}
For a codeword $u \in L$, we define the message $\operatorname{Dec}_\zeta(u)$ to be $\left(p_u\left(\zeta_1\right), \ldots, p_u\left(\zeta_k\right)\right)$, where $p_u$ is the polynomial (of degree $<k$ ) corresponding to $u$. For $U \in L^m$ with rows $u_1, \ldots, u_m \in L$, we let $\operatorname{Dec}_\zeta(U)$ be the length-mk vector $x=\left(x_{11}, \ldots, x_{1 k}, \ldots, x_{m 1}, \ldots, x_{m k}\right)$ such that $\left(x_{i 1}, \ldots, x_{i k}\right)=\operatorname{Dec}\left(u^i\right), i \in[m]$. Finally we say that $U$ encodes $x$ if $x=\operatorname{Dec}_\zeta(U)$, we use $\operatorname{Dec}(U)$ when $\zeta$ is clear from the context.

\end{frame}


\begin{frame}{Reed-Solomon Code Complexity}
	\begin{minipage}{0.42\linewidth}
	\includegraphics[scale=0.45]{rs2.png}
\end{minipage}
\end{frame}

\begin{frame}{Ligero}
We have a matrix that has $C$ elements. The dimension of this matrix is $\sqrt{C} \times \sqrt{C} $. The encoding of each row takes $\sqrt{C} \log{C}$. The overall
encode time is  $ \mathcal{O}(C \log{C})$. The communication time takes $t \times ColumnSize+RowSize=\mathcal{O}((t+1) \times \sqrt{C})$ time
\end{frame}

\begin{frame}{Ligero}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.35]{p11.png}
	\end{minipage}
\end{frame}
...
\begin{frame}{Ligero++}
	We set the size of matrix as $\frac{C}{polylog(C)} \times polylog(C) $
\end{frame}
...
\begin{frame}{Ligero}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.35]{p22.png}
	\end{minipage}
\end{frame}
...
\begin{frame}{Ligero}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.35]{p33.png}
	\end{minipage}
\end{frame}
...
\begin{frame}{Ligero++}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.45]{c3.png}
	\end{minipage}
\end{frame}
