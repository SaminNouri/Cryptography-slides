...
\begin{frame}{Inner product argument}
	Prover wants to prove the verifier that $<a,b>=y$:
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.75]{i1.png}
	\end{minipage}
\end{frame}

\begin{frame}{Sumckeck protocol}
	The goal of the protocol:
	$$\sum_{(x_1,\ldots,x_v) \in \{0,1\}^{v}} g(x_1,\ldots,g_v)=H$$
\end{frame}

\begin{frame}{Sumckeck protocol}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.30]{p1.png}
	\end{minipage}
\end{frame}


\begin{frame}{GKR protocol}
	\begin{minipage}{0.42\linewidth}
...
\begin{frame}{GKR protocol}
Assume we have a circuit $C$ with depth $D$. Layer ith has $M_i$ gates and we define $s_i=\log{M_i}$\\
Each $b \in \{0,1\}^{s_i}$ is a label for a specific gate in ith layer\\
\end{frame}
...
\begin{frame}{Transparent Zero Knowledge Polynomial Delegation}
	We want to evaluate an $l$ variate function $f$ with degree $d$ at $t=(t_0,\ldots,t_l)$.\\
	$W_i(x)$ is the ith monimial and $c_i$ is the ith coefficient of $f$.\\
	$f$ has $N=W_{l,d}=(d+1)^l$ monomials and coefficients. Thus, we have:
	$$f(x_1,\ldots,x_l)=\sum_{i=1}^{N}c_i \times W_i(x)$$
	$$T=(W_1(t),\ldots,W_N(t))$$
	$$f(x_1,\ldots,x_l)=\sum_{i=1}^{N}c_i \times T_i$$
\end{frame}

\begin{frame}{Transparent Zero Knowledge Polynomial Delegation}
We then select a multiplicative coset such that $|\mathbb{H}|=N$\\
We find polynomials such that:
$$l|_{\mathbb{H}}=c \quad q|_{\mathbb{H}}=T $$
$$f(x_1,\ldots,x_l)=\sum_{a \in \mathbb{H}}l(a) \times q(a)$$
The verifier can check equation using univarate sumcheck $\implies$ Due to low degree test, the verifier needs evaluations of $l(a) \times q(a)$ at $k$ points in a superset of $\mathbb{H}$ like $\mathbb{L}$ $\implies$ The prover can commit to $l|_{\mathbb{L}}$ with merkle tree. However, $T$ is a public vector and the verifier needs to evaluate it herself $\implies$ takes linear time
\end{frame}

\begin{frame}{Reducing the verification time}
	\begin{minipage}{0.42\linewidth}
...
\begin{frame}{Reducing the verification time}
	\begin{itemize}
		\item
$t_i$ is raised to the powers of $0,\ldots,d$. Monomials are evaluated using these values. The size of this part is $O(N)$ and the depth is $O(\log{d}+l)$
		\item
		We
		first construct a circuit for an inverse FFT to compute the coefficients of polynomial $q(x)$ from its evaluations $T$. Then  we run an FFT to evaluate $q|_{\mathbb{L}}$ from the coefficients of $q(x)$. The size of the circuit is $O(N \log{N})$ and the depth is $O(\log{N})$
		\item
		Finally, $k$ points are selected from $q|_{\mathbb{L}}$
	\end{itemize}
\end{frame}

\begin{frame}{Reducing the verification time}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.45]{r4.png}
	\end{minipage}
\end{frame}

\begin{frame}{Reducing the verification time}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.35]{r3.png}
	\end{minipage}
\end{frame}

\begin{frame}{Reducing the verification time}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.22]{t1.png}
	\end{minipage}
\end{frame}
...
\begin{frame}{Testing Linear Constraint Ligero}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.60]{y1.png}
	\end{minipage}
\end{frame}
...
\begin{frame}{Testing Linear Constraint Ligero++}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.37]{i2.png}
	\end{minipage}
\end{frame}

\begin{frame}{Testing Linear Constraint Ligero++}
	The second check reduces the size of the complexity to the logarithmic to the size of columns. We set the size of the matrix to $\frac{|C|}{polylog(|C)} \times polylog(|C|)$ $\implies$ the size of the complexity of the second check is  $polylog(|C|)$ and the size of the first check is $polylog(|C|)$
\end{frame}


\begin{frame}{Testing Linear Constraint Ligero++}
	\begin{minipage}{0.42\linewidth}
		\includegraphics[scale=0.55]{i3.png}
	\end{minipage}
\end{frame}
